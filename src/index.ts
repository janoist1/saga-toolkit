...
const wrap = (saga: SagaWorker) => function* (action: { meta: { requestId: string } }, ...rest: unknown[]): Generator<unknown, void, unknown> {
...
const takeEveryHelper = (patternOrChannel: ActionPattern | Channel<unknown>, worker: SagaWorker, ...args: unknown[]) => fork(function* () {
    while (true) {
        const action = (yield take(patternOrChannel as ActionPattern)) as Action
...
export function takeLatestAsync(pattern: ActionPattern | Channel<unknown>, saga: SagaWorker, ...args: unknown[]) {
    ...
    function* wrapper(action: { meta: { requestId: string } }, ...rest: unknown[]): Generator<unknown, void, unknown> {
        if (deferred) {
        ...
        const { requestId } = (yield getRequest(action.meta.requestId)) as Request
    ...
    const customTakeEvery = (patternOrChannel: ActionPattern | Channel<unknown>, saga: SagaWorker, ...args: unknown[]) => fork(function* (): Generator<unknown, void, unknown> {
        while (true) {
            const action = (yield take(patternOrChannel as ActionPattern)) as { meta: { requestId: string } }
            const { requestId } = action.meta
